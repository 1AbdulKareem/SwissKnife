...
case $OE_VERSION in
  ...
  15.0)
    ...
    if [ ! -d "/odoo/15.0-venv" ]; then
      echo "Error: /odoo/15.0-venv directory does not exist. Exiting."
      exit 1
    fi
    OE_PATH=$DIR/$OE_VERSION
    VENV=$DIR/$OE_VERSION-venv
...
User=odoo-$USERNAME
Group=odoo
ExecStart=/home/odoo-$USERNAME/$OE_VERSION-venv/bin/python /odoo/$OE_VERSION/odoo-bin -c /odoo/conf.d/odoo-$OE_VERSION-$USERNAME
